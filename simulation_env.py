...
class peg_in_hole_env(object):
    ...
    def next_step(self, action, step, step_max, interval_):
        ...
        divisor1 = 0.0005 + interval/2
        divisor2 = 0.0005 + interval/2
        # divisor1 = 0.0002 + interval * 5 /(step + 5)
        # divisor2 = 0.0002 + interval * 5 /(step + 5)
        divisor3 = 0.00025
        # divisor3 = 0
        divisor = 0.2 + 100 * interval
        # divisor = 0
        limit = 2
    ...
    def reward_fun(self, action, state, f, step, step_max, interval, interval_):
        ...
        rf = 0.3 * (1-abs(f[0]) / 70) + 0.3 * (1-abs(f[1]) / 70) + 0.4 * (1-abs(f[2]) / 70)
        rt = 0.5 * (1-abs(f[3]) / 7) + 0.5 * (1-abs(f[4]) / 7)
        r3 = 0.3 * rt + 0.7 * rf
        # print("力：", r3)
